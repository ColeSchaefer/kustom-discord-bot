    ...
    commandCallback: async function(message, bot) {
        ...
        if (argv[1] && userTo) {
            if (message.member.hasPermission('MANAGE_MESSAGES')) { 
    			...
    			if (userTo.roles.has(role.id)) return message.channel.send(message.guild.member(message.author) + ' → ' + strings.GetString(settings.language, "USER_ALREADY_MUTED"));
    			...
    			await userTo.addRole(role);
    			
    			message.channel.send(message.guild.member(message.author) + ' → ' + strings.GetString(settings.language, "USER_HAS_BEEN_MUTED"));
    		}
        }
    }
};
